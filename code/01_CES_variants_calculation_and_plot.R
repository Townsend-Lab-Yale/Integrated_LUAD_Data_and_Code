...
### WGS data
NCI_maf <- preload_maf(maf = maf_list$NCI, refset = ces.refset.hg19, keep_extra_columns = T) ## chain_file = liftover_file, 
NCI_maf <- NCI_maf[is.na(problem)]
NCI_maf <- NCI_maf[germline_variant_site == F & (repetitive_region == F | cosmic_site_tier %in% 1:3)]

### WES+WGS
Broad_maf <- preload_maf(maf = maf_list$Broad, refset = ces.refset.hg19,  keep_extra_columns = T) ## chain_file = liftover_file,
...
### WES
MSK2015_maf <- preload_maf(maf = maf_list$MSK2015, refset = ces.refset.hg19,  keep_extra_columns = T)## chain_file = liftover_file,
...
OncoSG_maf <- preload_maf(maf = maf_list$OncoSG, refset = ces.refset.hg19, keep_extra_columns = T) ## chain_file = liftover_file, 
...
TracerX_maf <- preload_maf(maf = maf_list$TracerX, refset = ces.refset.hg19,  keep_extra_columns = T) ## chain_file = liftover_file,
...
### WES; TGS
MSK2018_maf <- preload_maf(maf = maf_list$MSK2018, refset = ces.refset.hg19, keep_extra_columns = T) 
...
Genie_maf <- preload_maf(maf = maf_list$Genie, refset = ces.refset.hg19, keep_extra_columns = T) 
Genie_maf <- Genie_maf[is.na(problem)]

MSK2017_maf <- preload_maf(maf = maf_list$MSK2017, refset = ces.refset.hg19, keep_extra_columns = T) 
MSK2017_maf <- MSK2017_maf[is.na(problem)]

TSP_maf <- preload_maf(maf = maf_list$TSP, refset = ces.refset.hg19, keep_extra_columns = T)
...
cesa <- load_maf(cesa, maf = FMAD_maf, coverage = 'targeted', 
                 ...
                 covered_regions_name = 'fmad_regions', covered_regions_padding = 100, maf_name = 'FMAD') #padding based on 23 variants having distance from interval between 10 and 100.
...
for(i in 1:length(Genie_maf)){
  cesa <- load_maf(cesa, maf = Genie_maf[i][[1]], coverage = 'targeted', 
                   ...
                   covered_regions_name = paste0(names(Genie_maf)[i], '_regions'), covered_regions_padding = 100,maf_name = names(Genie_maf)[i])
}

cesa <- load_maf(cesa, maf = MSK2017_maf$IMPACT341, coverage = 'targeted', covered_regions = paste0(location_bed,"msk341_targets.bed"), 
                 covered_regions_name = 'msk341_regions', covered_regions_padding = 100,maf_name = 'MSK2017_IMPACT341')
cesa <- load_maf(cesa, maf = MSK2017_maf$IMPACT410, coverage = 'targeted', covered_regions = paste0(location_bed,"msk410_targets.bed"), 
                 covered_regions_name = 'msk410_regions', covered_regions_padding = 100,maf_name = 'MSK2017_IMPACT410')
cesa <- load_maf(cesa, maf = MSK2018_maf$IMPACT341, coverage = 'targeted', covered_regions = paste0(location_bed,"msk341_targets.bed"), 
                 covered_regions_name = 'msk341_regions', covered_regions_padding = 100,maf_name = 'MSK2018_IMPACT341')
cesa <- load_maf(cesa, maf = MSK2018_maf$IMPACT410, coverage = 'targeted', covered_regions = paste0(location_bed,"msk410_targets.bed"), 
                 covered_regions_name = 'msk410_regions', covered_regions_padding = 100,maf_name = 'MSK2018_IMPACT410')
cesa <- load_maf(cesa, maf = MSK2018_maf$IMPACT468, coverage = 'targeted', covered_regions = paste0(location_bed,"msk468_targets.bed"), 
                 covered_regions_name = 'msk468_regions', covered_regions_padding = 100,maf_name = 'MSK2018_IMPACT468')
cesa <- load_maf(cesa, maf = TSP_maf, coverage = 'targeted', covered_regions = paste0(location_bed,"tsp_targets.bed"), 
                 covered_regions_name = 'tsp_regions', covered_regions_padding = 100,maf_name = 'TSP')
...
all_treated_samples_for_sigs = all_samples[all_samples %in% clin_df[(Treatment)]$`Sample ID`]
all_untreated_samples_for_sigs = all_samples[all_samples %in% clin_df[(!Treatment)]$`Sample ID`]
all_treatment_not_indicated_samples_for_sigs = all_samples[all_samples %in% clin_df[is.na(Treatment)]$`Sample ID`]
sample_with_clinInfo <- c(all_treated_samples_for_sigs,all_untreated_samples_for_sigs,all_treatment_not_indicated_samples_for_sigs)
...
cesa <- trinuc_mutation_rates(cesa,
                              ...
                              samples = c(all_treated_samples_for_sigs, all_treatment_not_indicated_samples_for_sigs),
                              cores=4)
cesa <- trinuc_mutation_rates(cesa,
                              signature_set = ces.refset.hg19$signatures$COSMIC_v3.2,
...
sample_smo_wP_forCesa <- cesa_smoking_w_panel$samples[Unique_Patient_Identifier %in% c(intersect(smoking_samples, sample_with_clinInfo), intersect(panel_smoking_samples,sample_with_clinInfo)), Unique_Patient_Identifier] ## 1066
sample_nonsmo_wP_forCesa <- cesa_nonsmoking_w_panel$samples[Unique_Patient_Identifier %in% c(intersect(nonsmoking_samples, sample_with_clinInfo), intersect(panel_nonsmoking_samples,sample_with_clinInfo)), Unique_Patient_Identifier] ## 656
### calculate trinucleotide mutation rates for smoker#### 
cesa_smoking_w_panel <- trinuc_mutation_rates(cesa_smoking_w_panel,
                              signature_set = ces.refset.hg19$signatures$COSMIC_v3.2,
                              signature_exclusions = treated_samples_signature_exclusions, 
                              samples = intersect(c(all_treated_samples_for_sigs, all_treatment_not_indicated_samples_for_sigs), sample_smo_wP_forCesa),
                              cores=4)
cesa_smoking_w_panel <- trinuc_mutation_rates(cesa_smoking_w_panel,
                              signature_set = ces.refset.hg19$signatures$COSMIC_v3.2,
                              signature_exclusions = untreated_samples_signature_exclusions, 
...
cesa_nonsmoking_w_panel <- trinuc_mutation_rates(cesa_nonsmoking_w_panel,
                                              signature_set = ces.refset.hg19$signatures$COSMIC_v3.2,
                                              signature_exclusions = treated_samples_signature_exclusions, 
                                              samples = intersect(c(all_treated_samples_for_sigs, all_treatment_not_indicated_samples_for_sigs),sample_nonsmo_wP_forCesa),
                                              cores=4)
cesa_nonsmoking_w_panel <- trinuc_mutation_rates(cesa_nonsmoking_w_panel,
                                              signature_set = ces.refset.hg19$signatures$COSMIC_v3.2,
                                              signature_exclusions = untreated_samples_signature_exclusions, 
...
cesa_smoking_w_panel <- ces_variant(cesa = cesa_smoking_w_panel, run_name = "recurrents",
                                           samples = sample_smo_wP_forCesa)
length(unique(cesa_smoking_w_panel$samples[Unique_Patient_Identifier %in% sample_smo_wP_forCesa, Unique_Patient_Identifier])) ## 1066 samples
table(cesa_smoking_w_panel$samples[Unique_Patient_Identifier %in% sample_smo_wP_forCesa]$coverage, exclude = NULL)
...
cesa_nonsmoking_w_panel <- ces_variant(cesa = cesa_nonsmoking_w_panel, run_name = "recurrents",
                                              samples = sample_nonsmo_wP_forCesa)
length(unique(cesa_nonsmoking_w_panel$samples[Unique_Patient_Identifier %in% sample_nonsmo_wP_forCesa, Unique_Patient_Identifier])) ## 656 samples
table(cesa_nonsmoking_w_panel$samples[Unique_Patient_Identifier %in% sample_nonsmo_wP_forCesa]$coverage, exclude = NULL)
# exome   genome targeted 
# 304      190      162 
...
target_effect <- rbind(cesa_smoking_w_panel$selection$recurrent[oncogene_variants, on = "variant_name"][, group:= "Smoker"][],
                       cesa_nonsmoking_w_panel$selection$recurrents[oncogene_variants, on = "variant_name"][,group := "Never-smoker"][])
target_effect[,group := factor(group, levels = c("Never-smoker","Smoker"), ordered =T)] 
variant_order <- target_effect["Never-smoker", on = "group"][order(selection_intensity, decreasing = T)][,variant_name]
...
variant_label <- target_effect[, .(max_y = max(selection_intensity)), by = variant_name]
ggplot(target_effect, aes(x = variant_name, y = selection_intensity, fill = group)) +
  geom_bar(stat = "identity",color = "black",position = position_dodge(width = 0.9))+ ##"dodge"  这个设置不同 group 之间 SI 点分离的距离
  geom_errorbar(aes(ymin = ci_low_95, ymax = ci_high_95), color = "black", na.rm = T, width = 0.3, linewidth = 0.6, position = position_dodge(width = 0.9))+ ##position_dodge(width = 0.5)
  # Add variant_name labels above the bars
  geom_text(data = variant_label,
